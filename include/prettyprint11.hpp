...
namespace pretty_print {
  ...
  // SFINAE type trait to detect whether T::const_iterator exists.
    ...
    private:
      template <typename C>
      static yes &f(typename std::enable_if<std::is_same<
                    decltype(static_cast<typename C::const_iterator(C:: *)()
                             const>(&C::begin)),
  ...
  struct print_container_helper<T, TChar, TCharTraits, TDelimiters>::printer<
    std::pair<T1, T2>> {
    using ostream_type =
      typename print_container_helper<T, TChar, TCharTraits, TDelimiters>::
      ostream_type;
  ...
  struct print_container_helper<T, TChar, TCharTraits, TDelimiters>::printer<
    ...
    static void tuple_print(const element_type &,
                            ostream_type &,
                            Int<sizeof...(Args)>) {
    }
    ...
    static void tuple_print(const element_type &c,
                            ostream_type &stream,
                            typename std::conditional<sizeof...(Args) != 0,
                            Int<0>,
                            std::nullptr_t>::type) {
      stream << std::get<0>(c);
  ...
  const delimiters_values<char>
    delimiters<::std::unordered_set<T, THash, TEqual, TAllocator>,
    char>::values = { "{", ", ", "}" };
